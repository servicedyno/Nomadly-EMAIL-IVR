...
> Latest entry: **2026-06-18 — TRC20 $20 min-deposit bypass (string-comparison bug) + $60-vs-$30 dispute closed**. Root cause for the bypass: the intercept compared `ticker === 'trc20_usdt'` but `ticker` held the *display key* `'USDT (TRC20)'` (output of `supportedCryptoView[tickerView]`, not the internal ticker). Audit confirmed 5 sub-$20 TRC20 top-ups slipped through in last 14d, including ref `utC7I $15` on 2026-06-18 11:22 UTC. Fix routes the comparison through the canonical `tickerOf` map, with defense-in-depth re-check inside `showDepositCryptoInfo`. Regression test `/app/js/__tests__/trc20-min-deposit-intercept.test.js` (9/9 pass). Separately, the user 7191777173 $60-vs-$30 dispute is closed: full Mongo forensic shows all credits/debits reconcile (`usdIn=$60`, `usdOut=$60`, balance $0). No funds missing.
