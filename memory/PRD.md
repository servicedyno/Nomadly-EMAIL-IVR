...
> Latest entry: **2026-06-18 (cont.) — Addon-domain NS delegation + DynoPay forensic persistence**. (1) Fixed a P0 bug where OpenProvider addon-domain purchases skipped registrar NS delegation — CF zone created but registrar kept `ns1/2/3.openprovider.{nl,be,eu}`, hosting panel showed error. Live rescue applied for HHR2009 / `inviolivepaperless.com` (NS confirmed 2/2 CF in 3 s, zone now active, A→CF edge IPs). Code patch in `addon-domain-flow.js` + 9-assertion regression test. (2) Implemented DynoPay forensic persistence: every webhook payload now archived to `dynopayWebhooks` (365 d TTL); every deposit address + session frozen to `cryptoDepositAddresses` (no TTL) BEFORE the existing `del(chatIdOfDynopayPayment, ref)` cleanup runs; `payments[*].val` CSV extended with column-11 (address, backwards-compatible). Closes the post-hoc-traceability gap that made the 2026-06-18 deposit dispute slow to resolve. 36-assertion regression test.
