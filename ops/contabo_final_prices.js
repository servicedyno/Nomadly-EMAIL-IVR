...
async function rv() {
  const r = await fetch('https://backboard.railway.app/graphql/v2', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Project-Access-Token': RTOK },
...
;(async () => {
  ...
  const found = []
  for (const e of (prods.data || [])) {
    const pi = e.priceItem
    if (want.has(pi.itemId)) {
      const usd = (pi.price || []).find(x => x.currency === 'USD')?.amount
      const eur = (pi.price || []).find(x => x.currency === 'EUR')?.amount
      found.push(`${pi.itemId} | ${pi.name} | USD $${usd} | EUR €${eur}`)
    }
  }
  found.sort()
