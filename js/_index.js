...
All verified numbers generated during sourcing.`))
  ...
  if (action === 'quick-activate-domain-shortener') {
    ...
    try {
      ...
      if (cfEnsure.error) {
        log(`[QuickActivateShortener] ensureCloudflare failed for ${domain}: ${cfEnsure.error}`)
        // Non-fatal: proceed anyway, DNS will be added to whatever provider manages it
  ...
  if (action === 'crypto-pay-vps') {
    ...
    if (BLOCKBEE_CRYTPO_PAYMENT_ON === 'true') {
      const coin = tickerOf[ticker]
      const bbResult = await getCryptoDepositAddress(coin, chatId, SELF_URL, `/crypto-pay-vps?a=b&ref=${ref}&`)
      if (bbResult?.address) {
        ...
        log('[CryptoFallback] BlockBee unavailable for VPS, falling back to DynoPay')
        const dynoCoin = tickerOfDyno[ticker]
        const dynoResult = await getDynopayCryptoAddress(price, dynoCoin, `${SELF_URL}/dynopay/crypto-pay-vps`, { "product_name": dynopayActions.payVps, "refId": ref })
        if (!dynoResult?.address) return send(chatId, t.errorFetchingCryptoAddress, trans('o'))
      ...
      if (dynoResult?.address) {
        ...
        log('[CryptoFallback] DynoPay unavailable for VPS, falling back to BlockBee')
        const bbCoin = tickerOf[ticker]
        const { address: bbAddr, bb } = await getCryptoDepositAddress(bbCoin, chatId, SELF_URL, `/crypto-pay-vps?a=b&ref=${ref}&`)
        if (!bbAddr) return send(chatId, t.errorFetchingCryptoAddress, trans('o'))
        set(chatIdOfPayment, ref, { chatId, price, vpsDetails })
        log({ ref })
        await sendQrCode(bot, chatId, bb, info?.userLanguage ?? 'en')
  ...
  if (action === 'crypto-pay-plan') {
    ...
    if (BLOCKBEE_CRYTPO_PAYMENT_ON === 'true') {
      ...
      if (dynoResult?.address) {
        ...
        log('[CryptoFallback] DynoPay unavailable for plan, falling back to BlockBee')
        const bbCoin = tickerOf[ticker]
        const { address: bbAddr, bb } = await getCryptoDepositAddress(bbCoin, chatId, SELF_URL, `/crypto-pay-plan?a=b&ref=${ref}&`)
        if (!bbAddr) return send(chatId, t.errorFetchingCryptoAddress, trans('o'))
        set(chatIdOfPayment, ref, { chatId, price, plan })
        log({ ref })
        await sendQrCode(bot, chatId, bb, info?.userLanguage ?? 'en')
  ...
  if (action === 'confirm-switch-to-cloudflare') {
    ...
    try {
      ...
      if (result.error) {
        send(chatId, t.switchToCfError(sanitizeProviderError(result.error, 'domain')), { parse_mode: 'HTML' })
        return goto['choose-dns-action']()
      }
...
Please enter valid nameservers (e.g. ns1.example.com), one per line.`), { parse_mode: 'HTML' })
  ...
  const _payActions = ['phone-pay', 'domain-pay', 'hosting-pay', 'vps-plan-pay', 'vps-upgrade-plan-pay', 'digital-product-pay', 'virtual-card-pay']
...
Choose an IVR template category:`), k.of(rows))
  ...
  if (action === a.bulkTTSVoice) {
    ...
    const ttsService = require('./tts-service.js')
    const bulkTTS = info?.bulkTTS || {}
    const providerKey = bulkTTS.ttsProvider || 'openai'
    const voiceKey = ttsService.getVoiceKeyByButton(message)
    if (!voiceKey) {
      const voiceBtns = ttsService.getVoiceButtons('en', providerKey).map(b => [b])
  ...
  if (action === a.cpSelectArea) {
    ...
    const numBtns = results.map((_, i) => String(i + 1))
    return send(chatId, `📱 <b>Available Numbers — ${message}</b>\n\n${numberLines}\n\n☎️ = Supports Bulk IVR Campaigns\n\nTap a number to select:`, k.of([numBtns, [pc.showMore]]))
  }
  ...
  if (action === 'crypto-pay-phone') {
    ...
    if (BLOCKBEE_CRYTPO_PAYMENT_ON === 'true') {
      ...
      if (dynoResult?.address) {
        ...
        const bbCoin = tickerOf[ticker]
        const { address: bbAddr, bb } = await getCryptoDepositAddress(bbCoin, chatId, SELF_URL, `/crypto-pay-phone?a=b&ref=${ref}&`)
        if (!bbAddr) return send(chatId, t.errorFetchingCryptoAddress, trans('o'))
        set(chatIdOfPayment, ref, { chatId, price, cpData: { selectedNumber: info?.cpSelectedNumber, planKey: info?.cpPlanKey, provider: info?.cpProvider || 'telnyx', countryCode: info?.cpCountryCode || 'US', countryName: info?.cpCountryName || 'US' } })
        log({ ref })
        await sendQrCode(bot, chatId, bb, info?.userLanguage ?? 'en')
...
Professional templates for voicemail, customer support, financial institutions, and more. Select a category:`), k.of(catBtns))
    ...
    return send(chatId, ({ en: "Choose:", fr: "Choisissez :", zh: "请选择：", hi: "चुनें:" }[lang] || "Choose:"), k.of([[btn.useTemplate], [btn.typeText], [btn.uploadAudio]]))
  }
  ...
  if (action === a.cpVmGreetingVoice) {
    ...
    if (draft.text && draft.lang && draft.ttsProvider && !draft.voice) {
      ...
      try {
        ...
        return send(chatId, `❌ Audio generation failed: ${e.message}`, k.of([[btn.useTemplate], [btn.typeText], [btn.uploadAudio]]))
      }
    }
  ...
  if (action === a.cpIvrGreeting) {
    ...
    return send(chatId, `Choose an option:`, k.of([[btn.useTemplate], [btn.typeText], [btn.uploadAudio]]))
  }
...
What should happen when a caller presses <b>${key}</b>?`), k.of([[btn.forwardCall], [btn.playMessage], [btn.sendToVoicemail]]))
  }
...
How do you want to create the message?`), k.of([[btn.useTemplate], [btn.typeText], [btn.uploadAudio]]))
    }
    ...
    return send(chatId, ({ en: `Select an action for this key:`, fr: `Sélectionnez une action pour cette touche :`, zh: `选择此按键的操作：`, hi: `इस कुंजी के लिए कार्रवाई चुनें:` }[lang] || `Select an action for this key:`), k.of([[btn.forwardCall], [btn.playMessage], [btn.sendToVoicemail]]))
  }
...
async function resumeShortenerActivations() {
  ...
  try {
    ...
    log(`[ShortenerResume] Found ${tasks.length} incomplete activation(s) — resuming`)

    for (const task of tasks) {
      try {
        const { chatId, domain, status, server, recordType, lang: taskLang } = task
        ...
        log(`[ShortenerResume] Error resuming ${task.domain}: ${e.message}`)
        await markFailed(task.domain, e.message).catch(() => {})
      }
    }
  } catch (e) {
    log(`[ShortenerResume] Check error: ${e.message}`)
  }
}
