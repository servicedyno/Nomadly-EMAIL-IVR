 ...
 * 2. Auto-renew — charges wallet and extends plan (if auto-renew enabled & funds available)
 * 3. Suspend — suspends cPanel account if expired and insufficient funds
 * 4. Grace period — 48h grace after expiry before suspension
 *
...
function initScheduler(deps) {
  ...
  async function runCheck() {
    try {
      ...
      let renewed = 0, notified = 0, suspended = 0
      ...
      for (const account of allAccounts) {
        ...
        const isAutoRenew = account.autoRenew !== false // default true for legacy accounts

        // ── Case 1: Expiring within 24h — send advance notification ──
        if (expiry > now && expiry <= in24h && !account.expiryNotified) {
          const price = getPlanPrice(plan)
          const renewText = isAutoRenew
            ? `Your wallet will be auto-charged <b>$${price}</b> on expiry.`
            : `Auto-renew is <b>OFF</b>. Your account will expire and be suspended after ${GRACE_PERIOD_HOURS}h grace period.`
          ...
          notify(chatId,
            ...
            + `To toggle auto-renew: Go to My Hosting Plans → ${domain}`
          )
        ...
        // ── Case 2: Expired — attempt auto-renew or start grace period ──
        if (expiry <= now && !account.suspended) {
          const price = getPlanPrice(plan)
          ...
          if (isAutoRenew && price > 0) {
            ...
            if (balance >= price) {
              ...
              if (account.suspended) {
                ...
                await cpanelAccounts.updateOne({ _id: account._id }, { $set: { suspended: false } })
              }
              ...
              notify(chatId,
                ...
                + `Your account will be suspended in ${GRACE_PERIOD_HOURS}h if not renewed.`
              )
            }
          }

          // Not auto-renew or insufficient funds — check grace period
          if (expiry <= gracePeriodAgo) {
            // Grace period expired — suspend
            const suspendResult = await suspendAccount(account.cpUser, 'Plan expired — auto-renew failed or disabled')
            ...
            notify(chatId,
              ...
              + `<b>${domain}</b> has been suspended due to expired plan.\n\n`
              + `To reactivate: deposit funds and renew your plan, or contact support.`
            )
            suspended++
          }
        }
      ...
      if (renewed || notified || suspended) {
        log(`[HostingScheduler] Check complete: ${renewed} renewed, ${notified} notified, ${suspended} suspended`)
      }
    } catch (err) {
...
module.exports = { initScheduler, getPlanPrice, getPlanDuration }
