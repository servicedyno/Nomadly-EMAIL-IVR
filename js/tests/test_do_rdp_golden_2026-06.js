...
async function main() {
  ...
  ok('slow path attaches an install volume and records volume_id', slowCreate.data.volumes && slowCreate.data.volumes.length === 1 && (await db.collection('doRdpServers').findOne({ server_id: slow.instanceId })).volume_id === slowCreate.data.volumes[0])
  ...
  const srvLog = await db.collection('doRdpServers').findOne({ server_id: slow.instanceId })
